...
uint8_t composePacket(DualsenseData *dualData, DronePackSending *droneData){
	...
	if (comm[0] == 0xFF){ //OP MODE Bottone triangolo
		opcode = USE_MODE;
	...
	else if(comm[1] == 0xFF){ // GPS MODE Bottone quadrato
		opcode = USE_MODE;

		tempPayload = 0x01;
		len = 0x0100;
	}
	else if(comm[2] == 0xFF){ //ENGINE ON Bottone Cerchio
		opcode = ENGINE_ON;
		len = 0x0100;
		if (flymode == 0){
			if (engine == 0x00){
	...
	else if(comm[3] == 0xFF){ //FLY MODE ON Bottone R1
		opcode = FLY_ON;
		
		flymode = 1;

	}
	else if(comm[4] == 0xFF){ //FLY MODE OFF Bottone L1
		opcode = KEEP_ALIVE;
		flymode = 0;
	}
	else {
